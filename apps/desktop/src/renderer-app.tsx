  ...
  return <main className="relative isolate min-h-screen overflow-hidden bg-background text-foreground">
    ...
    {selected ? <section className="relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] max-w-[1500px] items-center justify-center px-6 pb-24">
      ...
      <div className="relative flex h-[min(67vh,680px)] w-full max-w-5xl items-center justify-center before:absolute before:inset-x-[12%] before:bottom-[-7%] before:h-16 before:rounded-full before:bg-primary/20 before:blur-3xl before:content-['']">
        {workspaces.map((item, index) => { const rawOffset = index - selectedIndex; const offset = rawOffset > workspaces.length / 2 ? rawOffset - workspaces.length : rawOffset < -workspaces.length / 2 ? rawOffset + workspaces.length : rawOffset; const isCenter = offset === 0; const isVisible = Math.abs(offset) <= 1; const presentation = presentations[item.root]; return <button key={item.root} onClick={() => isCenter ? choose(item) : setSelectedIndex(index)} aria-hidden={!isVisible} tabIndex={isVisible ? 0 : -1} className={`aimoto-carousel-card group absolute overflow-hidden rounded-[1.75rem] border text-left transition-all duration-500 ${isCenter ? "aimoto-carousel-center z-20 h-full w-[min(80%,980px)] border-primary/70 bg-card shadow-[0_24px_80px_color-mix(in_oklab,var(--primary)_20%,transparent)]" : isVisible ? "z-10 h-[78%] w-[min(62%,760px)] border-border bg-card opacity-35 shadow-xl hover:opacity-70" : "invisible pointer-events-none"}`} style={{ transform: `translateX(${offset * 58}%) scale(${isCenter ? 1 : .84})` }}>
          {presentation?.thumbnailUrl ? <img src={presentation.thumbnailUrl} alt="" className="relative z-0 h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.015]" /> : <div className="relative z-0 grid h-full place-items-center bg-muted"><span className="aimoto-display text-7xl text-primary/65">{item.name.slice(0, 1)}</span></div>}
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-10 bg-[linear-gradient(180deg,transparent_35%,color-mix(in_oklab,var(--background),transparent_5%)_100%)]" />
          <div className="absolute inset-x-0 bottom-0 z-20 p-7"><div><h1 className="aimoto-display text-2xl">{item.name}</h1><p className="mt-1 text-xs text-muted-foreground">rev {item.revision}</p></div></div>
        </button>; })}
      </div>
