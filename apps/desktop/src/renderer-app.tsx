  ...
  return <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_var(--aimoto-accent-soft),_transparent_38%)]">
    ...
    <section className="mx-auto flex max-w-6xl flex-col px-6 pt-[11vh]"><div className="relative h-[min(58vh,560px)]">{workspaces.map((ws, index) => {
      ...
      return <Card key={ws.root} onClick={() => setWorkspace(ws)} className={`absolute inset-x-0 mx-auto w-[min(52rem,86vw)] cursor-pointer transition-all duration-300 ${active ? "z-20 -translate-y-2 border-primary/60 shadow-2xl" : "z-10 translate-y-10 scale-[.94] opacity-60 hover:opacity-90"}`} style={{ top: `${index * 18}px` }}>
        <CardHeader><div className="flex items-start justify-between gap-4"><div className="flex gap-4"><div className="grid size-12 place-items-center rounded-lg bg-primary/10 text-lg font-semibold text-primary">{ws.name.slice(0, 1).toUpperCase()}</div><div><CardTitle className="aimoto-display text-xl">{ws.name}</CardTitle><CardDescription className="mt-1 line-clamp-2">Workspace revision {ws.revision}</CardDescription></div></div><div className="flex items-center gap-2"><Badge>{ws.currentVersionId ? `Version ${ws.currentVersionId}` : "No versions yet"}</Badge><Dialog><DialogTrigger asChild><Button aria-label={`Workspace settings for ${ws.name}`} variant="ghost" size="icon" onClick={event => event.stopPropagation()}><Settings2 className="size-4" /></Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle className="aimoto-display">{ws.name}</DialogTitle><DialogDescription>Actions are sent as a prompt to the active AI agent in the terminal. Nothing changes until you review and confirm it there.</DialogDescription></DialogHeader><div className="mt-8 grid gap-3"><Button variant="outline" onClick={() => sendPrompt(`Please rename this AI-Mo-To workspace from "${ws.name}" to: `)}>Rename workspace</Button><Button variant="outline" onClick={() => sendPrompt(`Please help me edit the notes for the AI-Mo-To workspace "${ws.name}". First show me the current notes.`)}>Edit workspace notes</Button><Button variant="outline" onClick={() => sendPrompt(`Please explain the safe, reversible steps to delete the AI-Mo-To workspace "${ws.name}". Do not delete anything until I explicitly confirm.`)}>Delete workspace</Button></div></DialogContent></Dialog></div></div></CardHeader>
        <CardContent><div className="rounded-md border bg-muted/30 p-4 text-sm text-muted-foreground">Open the workspace folder to use the application built there. New versions are created only after you explicitly approve the agent's version command.</div></CardContent>
      </Card>;
    })}</div></section>
