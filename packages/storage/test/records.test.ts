...
describe("household records", () => {
  it("keeps files and tasks across restart while rejecting stale or invalid writes", async () => {
    ...
    first.createRecord({ moduleId: "aimoto.files", collectionId: "file", recordId: "file-lease", now: at, data: {
      fileId: "file-lease", name: "Lease renewal.pdf", relativePath: "house/Lease renewal.pdf", kind: "file", size: 48219, updatedAt: at
    }});
    const draft = first.createRecord({ moduleId: "aimoto.files", collectionId: "file", recordId: "file-draft", now: at, data: {
    ...
    const task = first.createRecord({ moduleId: "aimoto.tasks", collectionId: "task", recordId: "task-call-landlord", now: at, validate: taskValidator, data: {
    ...
    expect(() => first.createRecord({ moduleId: "aimoto.tasks", collectionId: "task", recordId: "bad", now: at, validate: taskValidator, data: { title: "", status: "waiting" } })).toThrow(/needs a title/);
    first.close(); opened.pop();

    const restarted = open(database);
    expect(restarted.listRecords("aimoto.files", "file")).toHaveLength(1);
    expect(restarted.getRecord("aimoto.tasks", "task", task.recordId)?.data.status).toBe("done");
    expect(restarted.getRecord("aimoto.tasks", "task", task.recordId)?.version).toBe(2);
  });
});
